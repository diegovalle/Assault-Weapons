...
regions[["West"]] <- c("Nayarit", "Jalisco", "Colima", "Michoac·n")
regions[["East"]] <- c("Puebla", "Veracruz Llave", "Tlaxcala", "Hidalgo")
regions[["North East"]] <- c("Coahuila", "Nuevo LeÛn", "Tamaulipas")
regions[["South East"]] <- c("Tabasco", "Campeche", "Quintana Roo", "Yucat·n")
regions[["South West"]] <- c("Guerrero", "Oaxaca","Chiapas")
regions[["Center North"]] <- c("Aguascalientes", "Guanajuato", "QuerÈtaro", "San Luis PotosÌ", "Zacatecas")
regions[["Center South"]] <- c("Morelos", "MÈxico", "Distrito Federal")
...
mapply(function(x, y, z, height, width)
            savePlotAA(plotAsWeBreaks(homsub, breaks.df, ban, x, z), y,
                       height, width),
       list(NULL, "Chihuahua", "Nuevo LeÛn", "Michoac·n","Sonora",
            dts, st), filenames,
...
########################################################
nuevo.leon00 <- subset(homsub, homsub$State == "Nuevo LeÛn")
...
########################################################
nuevo.leon00 <- subset(hom, hom$State == "Nuevo LeÛn")
